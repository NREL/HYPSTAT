...
def get_demand(all_renewable_profiles, year, daily_demand, freq, file_path):
    ...
    #TODO: specify how user should format demand
    demand['total_demand'] = demand['h2_demand[kg/hr]']


    # Reshape demand data
    demand = demand.set_index(['Period', 'Network ID'])['total_demand'].unstack().sort_index()
    #TODO: handle different types of daily/monthly demands
    demand.index = demand.index.str.replace('D', '').astype(int)
